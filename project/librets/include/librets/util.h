...
bool startsWith(std::string aString, std::string prefix);
...
StringVectorPtr split(const std::string & aString,
                      const std::string & separators, unsigned limit = 0);

StringVectorPtr split_no_empties(const std::string & aString,
...
class ToUpper
{
  public:
    ...
    ToUpper() : loc(std::locale::classic()) {;}
...
int intValue(const std::string & aString);

std::string stringValue(int number);
